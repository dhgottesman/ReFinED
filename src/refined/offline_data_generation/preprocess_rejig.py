...
# train a new ReFinED ER model from cached fules 
...
def main():
    ...
    # Try to survive without sitelinks_cnt
    ...
    if False:   # Don't retrain - use old models
        ...
        if len([x[0] for x in list(os.walk(OUTPUT_PATH, followlinks=True)) if model_dir_prefix in x[0]]) == 0:
            logging.basicConfig(stream=sys.stdout, level=logging.INFO)
            ...
            train_md_model(resources_dir=OUTPUT_PATH, datasets=['onto', 'onto-article', 'onto-lower'],
                        device='cuda:0', max_seq=500, batch_size=64, bio_only=False, max_articles=None,
        ...
        if len([x[0] for x in list(os.walk(OUTPUT_PATH, followlinks=True)) if model_dir_prefix in x[0]]) == 0:
            ...
            train_md_model(resources_dir=OUTPUT_PATH, datasets=datasets,
                        device="cuda:0", max_seq=510, batch_size=64, bio_only=False,
        ...
        if not os.path.exists(os.path.join(OUTPUT_PATH, 'wikipedia_links_aligned_spans.json')):
            model_dir = [x[0] for x in list(os.walk(OUTPUT_PATH, followlinks=True)) if model_dir_prefix in x[0]][0]
